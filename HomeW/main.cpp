...
// Còðóêòóðà File_head äëÿ èçâëå÷åíèÿ èì¸í ôàéëîâ
...
#pragma pack(pop)
// Ïðîòîòèï ôóíêöèè âûâîäà èì¸í ôàéëîâ
...
int main() {
    ...
    HANDLE file_to_open_Handler = CreateFile("C:\\Users\\Àíäðåé\\Downloads\\HomeW\\Example (3).rar",
                                              GENERIC_READ,
                                              0,
                                              NULL,
                                              OPEN_EXISTING,
...
void PrintHeaderContent(const vector <char> & filebufer, DWORD filesize)
{
    ...
    // Ñîçäà¸ì ýêçåìïëÿð ñòðóêòóðû Volume Header
    cout << "\nFile size : " <<  filesize << endl ;
    Volume_Header header_main;
    // Ñîçäà¸ì ýêçåìïëÿð ñòðóêòóðû äëÿ header ôàéëà
    File_head file_header;
    int index_to_cut = sizeof(signatures); // 7 áàéò äëÿ ñðåçà(ñòðóêòóðà)
    ...
    wcout << "Êîëè÷åñòâî ôàéëîâ â àðõèâå :" << count_of_files;
